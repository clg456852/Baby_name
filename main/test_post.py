...
form = {
    ...
    'ctl00$ContentPlaceHolder1$InputBasicInfo1$tbXing': '郭',
    'ctl00$ContentPlaceHolder1$InputBasicInfo1$tbMingWords': '步驰',
...
def create_excel(result_file):
    ...
    for line in result_file:
        subs = line.split()
        ws.write(count, 0, subs[0])
        ws.write(count, 1, subs[1])
        ws.write(count, 2, subs[2])
        ws.write(count, 3, subs[3])
        ws.write(count, 4, subs[4])
        # ws.write(count, 0, n)
    ...
    wb.save('test_sheet2.xls')


lock = threading.Lock()
def write_names_score(name_score, source):
    global result_file
    if not name_score:
        return
    lock.acquire(3)
...
def core_func(n):
    ...
    if not n.startswith('来源') and len(n.strip()) != 0:
        ...
        write_names_score(name_score, new_source)
        if name_score:
            count += 1
...
if __name__ == '__main__':
    ...
    file_path = create_name.find_target_names(convertZh.chu_ci)
