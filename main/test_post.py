...
form = {
    ...
    "ctl00$ContentPlaceHolder1$InputBasicInfo1$tbMingWords": "庚",
...
if __name__ == '__main__':
    # global result_file
    # t_start = datetime.datetime.now()
    # target_path = create_name.find_target_names(convertZh.shi_jing)
    # file_path = create_name.attach_pre_name_to('withSrc_postive_', target_path)
    # dir_name, base_name = os.path.split(file_path)
    # result_file = dir_name + '/result_' + base_name
    #
    ...
    # t_end = datetime.datetime.now()
    # create_excel(result_file)
    # print "%f seconds to create names from %s" % ((t_end - t_start).seconds, os.path.split(result_file)[1])
    create_name_excel_from(convertZh.tang_300_peom)
    create_name_excel_from(convertZh.song_300_verse)
